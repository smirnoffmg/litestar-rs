...
идемпотентный хендлер. Библиотека даёт для этого dedup-ключ (`SET NX EX`), проверяемый
до исполнения, с явным выбором поведения при совпадении. Считать доставку ровно-однократной
